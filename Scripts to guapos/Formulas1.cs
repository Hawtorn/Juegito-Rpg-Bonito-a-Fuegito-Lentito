...
public static class Formulas
{
    ...
    {
        return Mathf.FloorToInt(10.0f * Mathf.Sqrt(stats.level)
            * stats.constitution);
    }

    public static int GetMaxMP(CharacterStats stats)
    {
        return Mathf.FloorToInt(Mathf.Sqrt(stats.level)
    ...
    public static int GetPhysicalDamage(CharacterStats attacker, CharacterStats defender, float multiplier)
    {
        float baseDamage = GetPhysicalAttackPower(attacker)
            / GetPhysicalDefensePower(defender) * multiplier;
    ...
    {
        float baseDamage = GetMagicalAttackPower(attacker)
            / GetMagicalDefensePower(defender) * multiplier;
        if (baseDamage < 1.0f)
        {
            baseDamage = 1.0f;
        }
